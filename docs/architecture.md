...
- 已接入行业指标映射和行业 provider v1.4：公告/财报规则抽取负责公司披露证据，行业 provider 补充铜价、铜链板块、保险同业、保险利率/AH 等外部读数；TC/RC、库存、升贴水等可通过铜链 CSV provider 补齐；任意股票/行业也可通过通用 `custom_metrics.csv` 注入用户自带指标；网页高级入口支持上传、下载、预览、删除和校验自定义行业 CSV。
