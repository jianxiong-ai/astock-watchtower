...
- 分析：临时输入任意 A 股，输出通用指标、行业特有指标、市场天气、公告事件、Stale Sources 和 Missing Inputs；若该股票已在订阅池中，会结合本地持仓基线输出盘中持仓操作纪律。
...
- FastAPI 后端
  ...
  - 持仓感知操作纪律：基于仓位权重、浮盈亏、市场天气、技术触发、公告和数据质量输出保守规则建议、执行步骤、风控线和下次复核清单
...
订阅推送会在有持仓基线时额外输出“持仓操作纪律”：

- 输入：持仓股数/成本/浮盈亏、组合内估算权重、市场天气、行情涨跌幅、技术信号、公告触发、Stale/Missing 数量。
- 输出：`持有` / `等待确认` / `分批减仓` / `条件式加仓` 四类主姿态，以及一句话纪律、触发条件、失效条件、执行步骤、风控线、下次复核清单、主要风险、下一决策点和可执行的 A 股整手数量范围。
- 原则：不会因为价格低于成本就机械补仓；当数据缺失、市场偏冷或仓位集中时，默认偏向等待确认或控制集中度。
- 边界：这是规则化研究提醒，不是自动交易；系统不会连接券商下单。未来可以把 LLM 摘要、预测模型或更高频策略作为外部/私有扩展接入，但开源核心默认保持可审计、可解释、可自托管。
...
- `report_sections` / `action_advice` / `position`：结构化报告、持仓纪律和持仓快照。
...
- 推送报告契约 v2：飞书推送和订阅页历史预览固定使用晨会卡片结构，先展示“晨会摘要、今日只看 3 件事、操作纪律和数据边界”，再展开详细证据层：交易日与市场温度、触发总览、单股市场快照、六组核心骨架、解释与验证链、公告与事件、持仓操作纪律、下一观察点。非日频指标不会消失，缺失项必须明确写为 Missing/Stale。
...
- 持仓与操作纪律：订阅推送会读取已上传交易记录生成的持仓基线，结合单票在组合内的估算权重、行情/技术/市场天气/公告/数据质量，输出保守规则建议、执行步骤、风控线和下一证据；无持仓基线时只输出研究监控姿态。
